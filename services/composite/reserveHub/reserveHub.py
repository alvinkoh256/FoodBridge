...
class ReserveHub(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            hub_info = self.get_hub_info(hub_id)
            ...
            if hub_info.get('isReserved', False):
                return {
                    "message": f"Hub '{hub_info.get('hubName', '')}' is already reserved",
                    "hubReserved": False
                }, 400
            
            # Step 2: Retrieve foodbank information
            foodbank_info = self.get_foodbank_info(foodbank_id)
            ...
            # Reserve the hub
            reserve_result = self.reserve_hub_in_service(reserve_payload)
...
class UnreserveHub(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if not hub_info.get('isReserved', False):
                return {
            ...
            unreserve_result = self.unreserve_hub_in_service({
                "hubID": hub_id,
                "foodbankID": foodbank_id
            })
    ...
    def get_foodbank_info(self, foodbank_id):
        ...
        try:
            ...
            # Get the first user (assuming the API returns a list)
            if not users:
                print(f"No user found for ID {foodbank_id}")
                return None
            
            user = users[0]  # Take the first user
            print(f"Retrieved user info: {json.dumps(user)}")
    ...
    def reserve_hub_in_service(self, reserve_payload):
    ...
    def unreserve_hub_in_service(self, unreserve_payload):
