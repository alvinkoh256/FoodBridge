<template>
     ...
     <div v-else class="listings-grid">
       <div v-for="(product, index) in listings" :key="product.productId" class="listing-card">
         <div class="status-indicator" 
              :class="{
                'status-ongoing': product.productStatus === 'on-going',
                'status-completed': product.productStatus === 'completed'
              }">
           {{ product.productStatus === 'on-going' ? 'In Progress' : 
              product.productStatus === 'completed' ? 'Completed' : 'Open' }}
         </div>
 ...
 .status-indicator {
   ...
   font-weight: bold;
   color: white;
   background-color: #4CAF50; /* Default: Open (green) */
   z-index: 1;
 }
 .status-ongoing {
   background-color: #FF5252; /* In Progress (red) */
 }
 .status-completed {
   background-color: #2196F3; /* Completed (blue) */
 }
 .listing-image {
   width: 100%;
   height: 200px;
